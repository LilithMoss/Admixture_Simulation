...
run.BMA <- function(d){
  ...
  #Keep Beta variance the same, but change alpha in cc model to sigma2
  cov1 <- matrix(sigma.1)
  cov2 <- matrix(c(sigma.2,0,0,sigma.1),ncol=2,byrow=T)
  ...
  #Make priors into vectors
  b0 <- c(1,1)
  a0 = c(1/(sigma.2) + 1,1/(sigma.1) + 1) #
  ...
  lterm1 <- exp(as(((-n/2)*log(2*pi)),"mpfr")) #1/(2pi)^(n/2)
  lterm2 <- list( exp(as(an[2]*log(bn[[1]]),"mpfr")), exp(as(an*log(bn[[2]]),"mpfr")) )
  lterm3 <- gamma(as(an,"mpfr"))
  lterm4 <- gamma(as(a0,"mpfr"))
  ...
  #New
  PrDGivenM1 <-lterm1*sqrt(det(lambda_0[[1]])/det(lambda_n[[1]]))*((b0^a0)/lterm2[[1]])*(lterm3/lterm4) #This is fine
  PrDGivenM2 <-lterm1*sqrt(det(lambda_0[[2]])/det(lambda_n[[2]]))*((b0^a0)/lterm2[[2]])*(lterm3/lterm4) 
